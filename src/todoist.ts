...
export class TodoistClient {
  ...
  async getComments(params: {
    ...
    return this.request<TodoistComment[]>(`/comments?${searchParams.toString()}`);
  }

  async getComment(commentId: string): Promise<TodoistComment> {
    return this.request<TodoistComment>(`/comments/${commentId}`);
  }
  ...
  async updateComment(
    ...
    return this.request<TodoistComment>(
      `/comments/${commentId}`,
      "POST",
      data
    );
